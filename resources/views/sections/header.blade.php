...
<header x-data="{ mobileOpen: false }" class="relative top-0 z-50 bg-white masthead fixed-top">
	...
	<!-- Desktop Header -->
	<div class="items-center justify-between hidden h-full py-4 px-6 lg:px-12 mx-auto md:flex">
		<a class="brand shrink-0" href="{{ home_url('/') }}">
			@if ($logo)
			<img src="{{ $logo['url'] }}" alt="{{ $logo['alt'] ?? 'Logo' }}" class="relative w-auto max-w-[140px] lg:max-w-[200px] h-12 -top-0.5">
			@else
		...
		<nav class="ml-4 lg:ml-15 nav-primary w-max" aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}">
			...
			'menu_class' => 'nav flex gap-x-4 lg:gap-x-8 text-sm font-medium items-center', // Usunięto 'nav-link' jeśli jest zbędne
			'container' => false,
			'echo' => false,
		...
		@endif
		<a class="small-btn" href="/formularz-wyceny/">Wypełnij formularz</a>
	</div>
	...
	<div
		...
		<div class="p-4 relative z-10">
			...
			<div class="mt-8">
				<a href="/formularz-wyceny/" class="block w-full second-btn">
					Wypełnij formularz
				</a>
			</div>
		</div>
